...
root.title('🦎')
def ァ():
    ア = simpledialog.askstring("ccshop2check.py", "paste the html from view-source:https://ccshop2.com/cc_list.php (check your terminal for the output)")
    if ア:
       イ = ア.split('\n')
       for a in イ:
           if '<td class="text-start">' in a:
            card = a.split('<td class="text-start">')[1].split('|')
            cc = card[0].strip(' ')
